...
jQuery(document).ready(($) => {
  ...
  $("#add-website-type").click(() => {
    var newType = "New Type " + (typeCounter + 1)
    $("#website-types-container").append(
      '<div class="website-type-row"><input type="text" name="website_types[]" value="' +
        newType +
        '" placeholder="Website Type"><button type="button" class="button remove-type">Remove</button></div>',
    )
    ...
    $("#website-type-tab-content").append(
      '<div class="wcc-tab-panel" id="tab-' +
        newIndex +
        '">' +
        "<h3>" +
        newType +
        " Options</h3>" +
        '<input type="hidden" name="website_type_key[]" value="' +
        newType +
        '">' +
        ...
        "</tr></thead>" +
        '<tbody class="calculator-options-container" data-type-index="' +
        newIndex +
        '"></tbody></table>' +
        '<button type="button" class="button add-calculator-option" data-type-index="' +
        newIndex +
        '">Add Option</button>' +
        "</div>",
    )
  ...
  $(document).on("click", ".add-calculator-option", function () {
    ...
    container.append(
      '<tr class="option-row">' +
        ...
        '<td><input type="number" name="option_hours[' +
        typeIndex +
        '][]" step="0.1" min="0" value="0" required></td>' +
        '<td><input type="number" name="option_price[' +
        typeIndex +
        '][]" step="0.01" min="0" value="0" required></td>' +
        '<td><input type="checkbox" name="option_default[' +
        typeIndex +
        "][" +
        optIndex +
        ']"></td>' +
        '<td><input type="checkbox" name="option_user_toggle[' +
        typeIndex +
        "][" +
        optIndex +
        ']"></td>' +
        '<td><input type="checkbox" class="base-field-checkbox" name="option_base_field[' +
        typeIndex +
        "][" +
        optIndex +
        ']"></td>' +
        '<td><input type="number" class="additional-field" name="option_additional_hours[' +
        typeIndex +
        "][" +
        optIndex +
        ']" step="0.1" min="0" value="0" disabled></td>' +
        '<td><input type="number" class="additional-field" name="option_additional_price[' +
        typeIndex +
        "][" +
        optIndex +
        ']" step="0.01" min="0" value="0" disabled></td>' +
